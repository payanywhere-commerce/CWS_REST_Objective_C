...
//  CWSAPI.m
//  CWSAPI
//  https://my.ipcommerce.com/Docs/1.17.16/CWS_REST_Developer_Guide/Introduction.aspx
...
#define BASE_URL @"https://cws-01.cert.ipcommerce.com/REST/2.0.17"
...
- (NSString *)signOn:(NSError **)error
{
    NSString *token = [self sendToURL:[NSString stringWithFormat:@"%@/SvcInfo/token", BASE_URL] withHTTPmethod:@"GET" andBodyJSONOfDict:nil requiresSession:NO error:error];
...
- (NSMutableDictionary *)buildTransactionWithCC:(CWSCreditCard *)creditCard andTransaction:(CWSTransactionData *)transaction error:(NSError **)error
{
    ...
    //'$type' : 'BankcardTransactionDataPro,http://schemas.ipcommerce.com/CWS/v2.0/Transactions/Bankcard/Pro',"
    ...
    if(transaction.creds != nil)
    {
        ...
        [transactionData setObject:addendum forKey:kAddenundum];
    }
...
- (NSDictionary *) authorizeTransaction:(CWSTransactionData *)transactionData withCreditCard:(CWSCreditCard *)creditCard error:(NSError **)error
{
    ...
    [postDictionary setObject:@"AuthorizeTransaction,http://schemas.ipcommerce.com/CWS/v2.0/Transactions/Rest" forKey:kAccountType];
    ...
    NSString *response = [self sendToURL:[NSString stringWithFormat:@"%@/Txn/%@", BASE_URL, self.serviceId] withHTTPmethod:@"POST" andBodyJSONOfDict:postDictionary requiresSession:YES error:error];
    InfoLog(@"RESPONSE = \n%@", response);
...
{
    NSError *error = nil;
    NSString *serviceInformation = [self sendToURL:[NSString stringWithFormat:@"%@/SvcInfo/serviceInformation", BASE_URL] withHTTPmethod:@"GET" andBodyJSONOfDict:nil requiresSession:YES error:&error];
...
- (NSDictionary *) captureTransactionWithId:(NSString *)transactionId forAmount:(NSNumber *)amount andTipAmount:(NSNumber *)tipAmount withTag:(NSString *)optionalTag  error:(NSError **)error
{
    ...
    NSMutableDictionary *differeceObject = [self buildDifferenceWithDifferenceData:differenceData];
    differeceObject[kAccountType] = @"BankcardCapture,http://schemas.ipcommerce.com/CWS/v2.0/Transactions/Bankcard";
    
    NSMutableDictionary *putDictionary = [[NSMutableDictionary alloc] init];
    putDictionary[kAccountType] = @"Capture,http://schemas.ipcommerce.com/CWS/v2.0/Transactions/Rest";
    putDictionary[kApplicationProfileId] = self.applicationProfileId;
    putDictionary[kDifferenceData] = differeceObject;
    InfoLog(@"Generated Dict: \n%@",putDictionary);

    NSString *response = [self sendToURL:[NSString stringWithFormat:@"%@/Txn/%@/%@",BASE_URL, self.serviceId, transactionId] withHTTPmethod:@"PUT" andBodyJSONOfDict:putDictionary requiresSession:YES error:error];
...
- (NSDictionary *) adjustTransactionWithId:(NSString *)transactionId withDifferenceData:(CWSDifferenceData *)difference withTag:(NSString *)optionalTag error:(NSError **)error
{
    ...
    NSMutableDictionary *putDictionary = [[NSMutableDictionary alloc] init];
    putDictionary[kAccountType] = @"Adjust,http://schemas.ipcommerce.com/CWS/v2.0/Transactions/Rest";
    putDictionary[kApplicationProfileId] = self.applicationProfileId;
...
- (NSDictionary *) undoTransactionWithId:(NSString *)transactionId withTag:(NSString *)optionalTag error:(NSError **)error
{
    NSMutableDictionary *putDictionary = [[NSMutableDictionary alloc] init];
    putDictionary[kAccountType] = @"Undo,http://schemas.ipcommerce.com/CWS/v2.0/Transactions/Rest";
    putDictionary[kApplicationProfileId] = self.applicationProfileId;
    putDictionary[kDifferenceData] = [NSNull null];
    InfoLog(@"Generated Dict: \n%@",putDictionary);
...
- (NSDictionary *) returnTransactionWithId:(NSString *)transactionId forAmount:(NSNumber *)amount withTag:(NSString *)optionalTag error:(NSError **)error
{
    CWSDifferenceData *differenceData = [[CWSDifferenceData alloc] init];
    differenceData.amount = amount;
    differenceData.transactionID  = transactionId;
    NSMutableDictionary *differenceDict = [self buildDifferenceWithDifferenceData:differenceData];
    differenceDict[kAccountType] = @"BankcardReturn,http://schemas.ipcommerce.com/CWS/v2.0/Transactions/Bankcard";
    
    NSMutableDictionary *postDictionary = [[NSMutableDictionary alloc] init];
    postDictionary[kAccountType] = @"ReturnById,http://schemas.ipcommerce.com/CWS/v2.0/Transactions/Rest";
    postDictionary[kApplicationProfileId] = self.applicationProfileId;
    postDictionary[kMerchantProfileId] = self.merchantProfileId;
...
- (NSDictionary *) returnUnlinkedTransaction:(CWSTransactionData *)transactionData withCreditCard:(CWSCreditCard *)creditCard error:(NSError **)error
{
    ...
    NSMutableDictionary *postDictionary = [[NSMutableDictionary alloc] init];
    [postDictionary setObject:@"ReturnTransaction,http://schemas.ipcommerce.com/CWS/v2.0/Transactions/Rest" forKey:kAccountType];
    [postDictionary setObject:self.applicationProfileId forKey:kApplicationProfileId];
    [postDictionary setObject:self.merchantProfileId forKey:kMerchantProfileId];
    [postDictionary setObject:transactionDataGenerated forKey:kTransaction];
    InfoLog(@"Generated Dict: \n%@",postDictionary);
...
- (NSDictionary *) captureSelectiveTransactionsWithIds:(NSArray *)transactionIds withDifferenceData:(NSArray *)differences withTag:(NSString *)optionalTag error:(NSError **)error
{
    ...
    if (differences)
    {
        ...
        for (int i=0 ; i < [transactionIds count]; i++)
        {
            ...
            differenceDict[kAccountType] = @"BankcardCapture,http://schemas.ipcommerce.com/CWS/v2.0/Transactions/Bankcard";
            [differenceObjects addObject:differenceDict];
        }
    }
    
    NSMutableDictionary *putDictionary = [[NSMutableDictionary alloc] init];
    putDictionary[kAccountType] = @"CaptureSelectiveAsync,http://schemas.ipcommerce.com/CWS/v2.0/Transactions/Rest";
    putDictionary[kApplicationProfileId] = self.applicationProfileId;
    ...
    NSString *response = [self sendToURL:[NSString stringWithFormat:@"%@/Txn/%@",BASE_URL, self.serviceId] withHTTPmethod:@"PUT" andBodyJSONOfDict:putDictionary requiresSession:YES error:error];
...
- (NSDictionary *) captureAllTransactionsWithDifferenceData:(NSArray *)differences withTag:(NSString *)optionalTag error:(NSError **)error
{
    ...
    NSMutableDictionary *putDictionary = [[NSMutableDictionary alloc] init];
    putDictionary[kAccountType] = @"CaptureAllAsync,http://schemas.ipcommerce.com/CWS/v2.0/Transactions/Rest";
    putDictionary[kApplicationProfileId] = self.applicationProfileId;
...
{
    NSMutableDictionary *postDictionary = [[NSMutableDictionary alloc] init];
    postDictionary[kAccountType] = @"QueryBatch,http://schemas.ipcommerce.com/CWS/v2.0/DataServices/TMS/Rest";
    ...
    NSString *response = [self sendToURL:[NSString stringWithFormat:@"%@/DataServices/TMS/batch",BASE_URL] withHTTPmethod:@"POST" andBodyJSONOfDict:postDictionary requiresSession:YES error:error];
